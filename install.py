...
def install():
    ...
    if not launch.is_installed("tensorrt"):
        ...
        launch.run_pip(
            "install nvidia-cudnn-cu12==9.5.1.17 --no-cache-dir", "nvidia-cudnn-cu12"
        )
        launch.run_pip(
            "install --no-cache-dir --extra-index-url https://pypi.nvidia.com tensorrt",
            "tensorrt",
            live=True,
        )
        launch.run(
    ...
    #     if version("nvidia-cudnn-cu12") == "9.5.1.17":
    #         launch.run(
    #             ["python", "-m", "pip", "uninstall", "-y", "nvidia-cudnn-cu12"],
    #             "removing nvidia-cudnn-cu12",
    #         )
